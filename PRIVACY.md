...
- Task history;
- transient session state, including any legacy plaintext relay-session token.

Generic privacy deletion intentionally cannot erase device identity or pairing state. Those records are security identities and require the dedicated device-reset flow rather than a broad history-delete endpoint. Deletion refuses symbolic-link traversal and does not wildcard-delete arbitrary paths.

The dedicated local device-reset endpoint requires the normal local-agent credential plus the separate recovery credential. When relay authority is configured, reset first obtains a hosted completion receipt for the currently authenticated device generation. Only after hosted revocation completes does local reset remove the protected relay credential, any legacy relay token, relay cursor/recovery state, local session state, pairing/routing state, old authority-bound approvals, relay outbox state and the local device private identity. Activity history, Task history, bootstrap/authorized-root configuration and emergency-stop state are preserved. A crash after hosted revocation can resume the bounded local cleanup from its local reset journal without recreating the old authority. Read-only identity/status operations do not silently mint a replacement identity; subsequent enrollment creates a new device UUID and a new Ed25519 key pair.

The public relay enforces a 24-hour default retention window for sensitive queued action payloads and for unconsumed returned results. Acknowledged delivery payloads are erased immediately. Pending deliveries that remain unacknowledged for 24 hours expire into payload-free sequence tombstones and are not later executed. A successfully consumed result is returned once and then its sensitive payload is physically removed immediately, leaving only bounded delivery/sequence/hash/timestamp tombstone metadata. Unconsumed result payloads expire after 24 hours by default. Startup pruning and periodic garbage collection physically remove expired result payloads without requiring a later read. These defaults are configurable only within bounded runtime limits and the production privacy notice must state the actual deployed values.

Authenticated hosted-account erasure removes the hosted account identity binding, account/device memberships, routing state and retained relay delivery/result/session authority. Hosted account erasure does not by itself claim to erase the private cryptographic identity stored on the user's computer. That local identity is non-authorizing after hosted revocation and remains local until the user performs the dedicated local device-reset flow or otherwise removes the Operator state. Releasing, resetting or erasing a hosted device membership purges retained delivery payloads, returned results, relay sessions and enrollment authority before that hosted membership can be reused.
