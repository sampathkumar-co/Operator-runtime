...
Production authentication policy and users were not changed by those dry runs.

### Production activation evidence — 2026-09-18

A high-entropy reviewer credential was generated by the pinned production Authelia CLI using its random-password mode; only the generated Argon2id digest was inserted into the file-user database. The reviewer user record and custom authorization-policy configuration were validated before activation. Backups of both production configuration and user database were created, only `operator-auth` was recreated, and the resulting container is healthy. The custom client policy defaults to two-factor and grants one-factor only to the dedicated reviewer group.

The reviewer password value was not printed into chat, Git, evidence Markdown or tool-visible output. Credential-handling safety controls prevented this certification agent from retrieving/decrypting it for a login test. Therefore production login, no-secondary-verification behavior, fixture-only pairing, and the real ChatGPT reviewer journey remain unproven and H2 remains BLOCKED.
