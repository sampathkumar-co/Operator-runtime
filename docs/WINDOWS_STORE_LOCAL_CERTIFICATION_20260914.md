...
## Final production-notice path-authority hardening

Exact-head Codex review of remote head `1d815e12247aac01486af744d13cf0942e19cbd6` found two remaining P2 filesystem-boundary issues in the production notice loader: validation and reading used separate path lookups, creating a replace-with-link/read TOCTOU window, and a symlinked notice directory or linked ancestor could evade the raw-path `lstatSync()` check after path normalization.

The notice loader now treats the opened file descriptor as the read authority. It validates every directory path component, requires the configured directory to canonicalize to itself, opens each fixed notice filename with `O_NOFOLLOW` where the platform exposes it, verifies the opened descriptor is the same stable regular file selected by pre-open validation, reads through that descriptor with an intrinsic `MAX_NOTICE_BYTES + 1` bound, revalidates descriptor stability after the read, and confirms the path still names the opened file before accepting the bytes. Windows uses the same opened-handle and stability checks plus linked-component rejection; Linux CI additionally exercises native `O_NOFOLLOW` file-symlink rejection.

Adversarial coverage now includes trailing-separator directory links, linked ancestor components, intrinsically bounded descriptor reads, repository-draft rejection, and a linked notice-file regression. Local certification on this P2-fixed tree: MCP **59 total / 58 passed / 0 failed / 1 expected Windows file-symlink privilege skip**, relay **36/36**, red-team **6/6**, deployment invariants **5/5**, root Windows suite **309 total / 293 passed / 0 failed / 16 expected environment skips**, all five real Windows junction tests executed, path-lease self-test green, strict TypeScript check green, syntax/import checker green, and `git diff --check` green. The skipped notice-file symlink case is expected to execute on the Linux clean runner rather than skip.

## Final capacity-exhaustion hardening (2026-09-15)

The final exact-head Codex pass identified two capacity risks. Rotated session predecessors are now retained only while the predecessor credential can still be presented for lost-response rotation recovery; after that expiry they are compacted, while non-rotation revocations retain the longer recovery/audit window. A 200-rotation regression proves automatic refresh history stays bounded.
