...
Relay result delivery is now receipt-based: completed results are read non-destructively until the MCP-side caller receives and parses the response, retries reuse the same durable delivery through an action-instance-bound idempotency key, forged receipt acknowledgements cannot consume results, successful receipt ACK tombstones the result payload before releasing dedupe, and duplicate ACKs remain safe.

Final local certification on this P1-hardened tree: relay package 31/31, MCP TypeScript/E2E 51/51, red-team 6/6, root Windows suite 303 total / 287 passed / 0 failed / 16 expected environment skips, all five real junction tests executed, path-lease self-test green, syntax/import checker green, and `git diff --check` green.
