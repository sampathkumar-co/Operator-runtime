...
The public edge now exposes exactly those three read-only paths and no general file-serving surface. Their content is sourced from the repository-reviewed `PRIVACY.md`, `TERMS.md`, and `SUPPORT.md` files bundled into the hardened container. The renderer HTML-escapes the complete source before embedding it in a bounded styled document, so repository Markdown/raw HTML cannot create an executable HTML/script surface. The routes are enabled only in public-edge mode and use the same Host/Origin validation as OAuth discovery and MCP ingress. Caddy explicitly proxies only `/privacy`, `/terms`, and `/support` to the MCP edge.
