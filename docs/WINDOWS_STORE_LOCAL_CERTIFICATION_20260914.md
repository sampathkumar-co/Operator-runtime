...
Relay result recovery is now invocation-replay based. The MCP JSON-RPC request ID is bound to a bounded scope and hashed into `taskId`; the relay idempotency key already binds account, action ID, task ID, canonical action hash, and public-boundary state. Completed results and their idempotency authority remain replayable for the bounded retention window, retry lookup happens before any online-device routing, and the same invocation can recover after relay/MCP restart even when the executing device is offline. A deliberate later invocation receives a different MCP request ID/task ID and therefore a distinct delivery. Idempotency authority expires from delivery creation time so it cannot outlive the replayable result window.

Final local certification on this recovery-hardened tree: relay package 31/31, MCP TypeScript/E2E 52/52, red-team 6/6, root Windows suite 303 total / 287 passed / 0 failed / 16 expected environment skips, all five real junction tests executed, path-lease self-test green, syntax/import checker green, and `git diff --check` green.
