...
Draft PR #22 implements the policy-neutral release machinery at head `f2de47d05582dd117974de97c908938999c4ad65`: first-release artifact handoff, exact-tarball staging for later versions, `UNLICENSED` fail-closed behavior, shipped-license verification, root `DISCLOSURE` packaging/enforcement, and continuity protection that reads the full currently published npm `/latest` manifest so an existing dual-use declaration cannot be silently removed by the next release. CI #690, Platform Matrix #460, NPM Runtime #119 and Signing Smoke #461 all pass on that exact head; local targeted release/policy tests are 16/16 PASS, both owner-selectable packaging shapes were simulated successfully, and npm 11.19.1 accepted the exact local tarball form in stage dry-run. `OPERATOR_NPM_NAMESPACE_BOOTSTRAP.md` remains the owner checklist for proving/creating the `mecrod` npm organization, enforcing 2FA, publishing the exact first tarball and retaining non-secret evidence. Keep the PR draft until the owner decisions below are applied and the resulting final successor is re-certified.
