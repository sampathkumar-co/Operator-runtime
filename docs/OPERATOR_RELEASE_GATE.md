...
| G19 Packaging / supply chain | BLOCKED | PR #22 / OCC-3N head `fbd5e24542c81ca4f6bed4a5f8d49cf7f772e2da` removes direct `npm publish`, provides an immutable artifact-only first-release path and exact-tarball staged path for later versions, and passes CI #659, Platform Matrix #429, NPM Runtime #98 and Signing Smoke #430. G19 remains BLOCKED until FG-013 licensing and FG-015 npm-policy classification are applied to the final successor, `@mecrod` ownership/2FA is proven, and affected publish/install certification is rerun. |
