...
| G19 Packaging / supply chain | BLOCKED | PR #22 / OCC-3N head `f2de47d05582dd117974de97c908938999c4ad65` removes direct `npm publish`, provides an immutable artifact-only first-release path and exact-tarball staged path for later versions, makes root `DISCLOSURE` packable, enforces current-published dual-use continuity, and passes CI #690, Platform Matrix #460, NPM Runtime #119 and Signing Smoke #461. Local targeted release/policy tests are 16/16 PASS and npm 11.19.1 accepts the exact tarball form for staged publishing in dry-run. G19 remains BLOCKED until FG-013 licensing and FG-015 npm-policy classification are applied to the final successor, `@mecrod` ownership/2FA is proven, and affected publish/install certification is rerun. |
