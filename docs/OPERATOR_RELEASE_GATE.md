...
| G19 Packaging / supply chain | BLOCKED | Build/dependency/SBOM/MSIX/signing evidence passes, but the public npm publication policy path is not final: npm dual-use classification is unresolved and the current workflow performs direct publication. Because `npm stage` requires an existing package, a successor must provide an artifact-only first-release path for interactive 2FA publication and a staged path for later releases before G19 can pass. |
