...
| G23 Real ChatGPT E2E | BLOCKED | Real ChatGPT OAuth reconnect completed and a fresh conversation exposes exactly 9 tools. `computer.inspect` reached production Mecord but returned `ROUTE_NO_DEVICE` because the paired local runtime was offline; device-backed read/write plus revoke/reconnect proof remain. |
| G24 OpenAI Scan Tools | BLOCKED | Must be run from the OpenAI submission portal against the exact production MCP endpoint. |
| G25 Reviewer simulation | BLOCKED | Local reviewer semantics remain certified and the public npm runtime is now available. Remaining blockers are reviewer credential verification, fixture-only device pairing and the device-backed real production reviewer journey; the latest live probe stopped at `ROUTE_NO_DEVICE`. |
