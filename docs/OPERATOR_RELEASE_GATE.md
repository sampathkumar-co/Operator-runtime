...
| G22 Legal / public docs | BLOCKED | Hosted Operator `/privacy`, `/terms`, `/support` pages are live, but the intended public npm package is `UNLICENSED` and the exact legal publisher identity is not yet disclosed consistently. Operator pages say "the publisher of splcart.in"; the main SPLCART Terms still call for review against the actual business entity/jurisdiction. Owner-approved software licensing **and** exact public publisher identity/legal-page reconciliation are required before public distribution/submission. |
