...
- The public registry currently returns E404 for `mecord-connect@1.0.0`.
...
6. immediately verify `npm view mecord-connect@1.0.0 version`, repository/homepage/license metadata and the public package page;
7. from a clean Windows x64 machine run `npx mecord-connect@latest doctor` and then the reviewer-root `remote` startup;
...
```powershell
npm org ls mecrod --json
npm view mecord-connect version repository homepage license --json
npm owner ls mecord-connect
```
