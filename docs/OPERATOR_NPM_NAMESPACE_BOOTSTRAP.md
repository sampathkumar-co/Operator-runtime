...
Target package: `@mecrod/connect@1.0.0`.

## What is known

- The public registry currently returns E404 for `@mecrod/connect@1.0.0`.
...
FG-013 licensing and FG-015 policy classification are already resolved in current source as **PROPRIETARY + DUAL_USE**. After the final successor SHA is frozen and the owner npm account/scope/2FA gate is ready:
...
6. immediately verify `npm view @mecrod/connect@1.0.0 version`, repository/homepage/license metadata and the public package page;
7. from a clean Windows x64 machine run `npx @mecrod/connect@latest doctor` and then the reviewer-root `remote` startup;
...
```powershell
npm org ls mecrod --json
npm view @mecrod/connect version repository homepage license --json
npm owner ls @mecrod/connect
```
