...
Routing is deterministic and fail-closed. Precedence is explicit device, project binding, account default, then a uniquely eligible device. Pairing may explicitly select the new computer as the account default; existing project bindings continue to take precedence. A configured project binding or default never silently fails over to another machine, revoked/offline/stale/capability-mismatched devices are rejected, and multiple eligible devices without any configured selection produce ambiguity rather than random selection. Project bindings use opaque logical keys and do not expose one machine's filesystem paths to another.
...
The Windows release builder compiles the native launcher, UIA sidecar and DPAPI helper, bundles a pinned Node runtime, the required Operator source closure, the MCP server with its locked production dependencies, and a protected one-command bootstrap CLI that verifies configuration and starts the local agent/MCP runtime. It generates an MSIX manifest with an `operator.exe` execution alias, Windows package-integrity enforcement and HTTPS `.appinstaller` metadata, packs with the Windows SDK, and records exact size/SHA-256/package identity metadata. The Microsoft Store lane uses the exact Partner Center identity and the CI-produced Store submission MSIX; current Microsoft Store requirements do not require a publisher-supplied CA-trusted certificate for MSIX/AppX submissions because the Store re-signs accepted packages during publishing. A separate zero-dependency `operator-runtime-cli` bootstrap remains a direct/sideload distribution lane and therefore keeps its stronger local signer-fingerprint, signer-subject, timestamp and minimum-version checks.

Direct-distribution code-signing mechanics are independently certified using an ephemeral CI certificate. The signing script enforces SHA-256, requires certificate subject to match the MSIX Publisher, verifies the resulting Authenticode signature, and requires an HTTPS RFC 3161 timestamp in production mode. The Windows smoke gate creates temporary trust material, builds a matching-publisher package, signs and verifies it, installs it with Windows package deployment, runs the installed launcher self-test, executes protected `setup`/`verify`, starts the installed runtime, verifies both local-agent and MCP health, uninstalls it and removes the temporary trust material. The direct production workflow is intentionally separate from Partner Center submission and currently fails closed until a real signer compatible with its package identity is configured and pinned. This direct-signing gate is not a Microsoft Store submission prerequisite.
