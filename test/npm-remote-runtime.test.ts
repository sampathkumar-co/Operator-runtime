...
} from '../packages/mecrod-connect/src/cli.mjs';
...
test('remote launcher binds its local agent to an ephemeral loopback port', async () => {
  const source = await fs.readFile(path.resolve('packages/mecrod-connect/src/cli.mjs'), 'utf8');
...
test('npm publication fails closed unless exact source commit is supplied', async () => {
  const source = await fs.readFile(path.resolve('packages/mecrod-connect/src/publish-check.mjs'), 'utf8');
...
test('npm release uses immutable first-release artifact and staged future publication', async () => {
  ...
  const sourceCommand = workflow.indexOf('node packages/mecrod-connect/src/publish-check.mjs', sourceGate);
  ...
  assert.match(workflow, /public npm release package name must be @mecrod\/connect/);
  ...
  assert.doesNotMatch(workflow, /npm publish \.\/packages\/mecrod-connect/);
});

test('runtime payload builder copies only tracked clean sources bound to HEAD', async () => {
  const source = await fs.readFile(path.resolve('packages/mecrod-connect/scripts/build-runtime-payload.mjs'), 'utf8');
...
test('npm package metadata is canonical before publication', async () => {
  const pkg = JSON.parse(await fs.readFile(path.resolve('packages/mecrod-connect/package.json'), 'utf8'));
  assert.equal(pkg.name, '@mecrod/connect');
  ...
  const readme = await fs.readFile(path.resolve('packages/mecrod-connect/README.md'), 'utf8');
